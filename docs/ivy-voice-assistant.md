# WaveShare P4 Touch LCD 4C — "Ivy" AI Screen

ESPHome config for the **Waveshare ESP32-P4 Wi-Fi6 Touch LCD 3.4C** running a full
voice-assistant + media-player UI with an animated avatar ("Ivy").
...
| 1 — Listening | Wake word detected | Ivy listening animation |
| 2 — Processing | STT end | Ivy processing animation |
| 3 — Replying | TTS start | Ivy replying animation |
| 4 — Now Playing | Music plays | Song title (scrolling) |

- Ivy animations use odd-numbered frames only (1, 3, 5 … 13) for 7-frame sequences
...
```
esphome-config/
  waveshare-p4-touch-lcd-4c-ai-ivy.yaml   ← main config (this device)
